...
export default function () {
  ...
  it('Get visible rows', function (done) {
    ...
    const visibleRows = grid.viewport.rows.size;
    done(assertIf(visibleRows !== 1, 'Expected 1 row to be returned'));
  });
  ...
  it('Goto a specific row', async function () {
    var grid = g({
      test: this.test,
      data: makeData(30, 30),
    });
    grid.gotoRow(20);
    assertCompareCondition(
      'after gotoRow(20), grid.scrollTop',
      grid.scrollTop,
      '>=',
      1,
    );
