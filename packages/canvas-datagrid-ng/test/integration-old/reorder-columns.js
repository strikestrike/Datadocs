...
export default function () {
  it('Hide columns by context menu item after reordering', async function () {
    ...
    const headerWidth = grid.sizes.columns[-1]?.size || baseWidth;
    ...
    const dnd = (x1, y1, x2, y2) => {
      ...
      mouseup(window, x1, y1, grid.canvas);
      // begin: Needs refreshed drag context
      // Drag context doesn't change unless the mouse moves. What we want is
      // it being updated as soon as the mouse is up. Right now, we only set
      // the context inside mousemove, hence the need for the line below.
      mousemove(window, x1, y1, grid.canvas);
      // end: Needs refreshed the drag context
      mousedown(grid.canvas, x1, y1);
    ...
    dnd(
      headerWidth + halfBaseWidth,
      10,
      headerWidth + baseWidth + halfBaseWidth,
      10,
    );
    await delay();
