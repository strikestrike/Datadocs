...
export default function () {
  ...
  it('Scroll vertically down via margin click until box capture', async function () {
    var grid = g({
      test: this.test,
      data: makeData(30, 20),
      scrollPointerLock: false,
  ...
  it('Scrolls to start if user jumps to a scrollable area from the frozen rows while drag selecting', function () {
    const grid = g({
      test: this.test,
      data: makeData(20, 20),
      ...
      style: {
        cellWidth: 50,
        cellHeight: 20,
        columnHeaderCellHeight: 20,
      }
    });

    grid.focus();
    grid.frozenRow = 2;
    grid.scrollIntoView(undefined, 10);
    doAssert(grid.scrollTop > 0, 'Should have scrolled after moving towards a cell');

    mousemove(window, 70, 30, grid.canvas);
    mousedown(grid.canvas, 70, 30);
    mousemove(window, 70, 75, grid.canvas);
    mouseup(window, 70, 75, grid.canvas);
    doAssert(grid.scrollTop === 0, 'Moves to top if crosses frozen row boundaries');
  });

  it('Scrolls to start if user jumps to a scrollable area from the frozen columns while drag selecting', function () {
    const grid = g({
      test: this.test,
      data: makeData(20, 20),
      allowFreezingRows: true,
    ...
    doAssert(grid.scrollLeft > 0, 'Should have scrolled after moving towards a cell');

    mousemove(window, 70, 30, grid.canvas);
    mousedown(grid.canvas, 70, 30);
    mousemove(window, 170, 30, grid.canvas);
    mouseup(window, 170, 30, grid.canvas);
    doAssert(grid.scrollLeft === 0, 'Moves to start if crosses frozen column boundaries');
  });

  it('Scrolls to start if user jumps to a scrollable area from the frozen columns while dragging fill handle', function () {
    const grid = g({
      test: this.test,
      data: makeData(20, 20),
      allowFreezingRows: true,
    ...
    handlemove(grid, 110, -10);
    doAssert(grid.scrollLeft === 0, 'Moves to start if crosses frozen column boundaries');
  });

  it('Scrolls to start after user auto-scrolls while in non-frozen area and returns to frozen area', function () {
    const grid = g({
      test: this.test,
      data: makeData(20, 20),
      allowFreezingRows: true,
    ...
    // Scroll horizontally while the user is still holding the mouse key.
    // This replicates the auto-scroll behavior here.
    grid.scrollLeft = 200;
    grid.draw();
    doAssert(grid.scrollLeft > 0, 'Should have scrolled horizontally');
