 ...
 */
export function delay(ms = 0, done = undefined) {
...
export function assertPxColorP(grid, x, y, expected, drawMarker) {
...
export function mousedown(el, x, y, bbEl, ev) {
  var p = bb(bbEl || el);
  ev = ev ?? {};
  de(el, 'mousedown', { ...ev, clientX: x + p.left, clientY: y + p.top });
}
...
/** @typedef {import('assert')} AssertModule **/
/** @returns {AssertModule} */
...
export function assertWithName(objectName, actual, expected) {
  try {
...
export function assertCompareCondition(xName, x, operator, y) {
