//@ts-check
/**
 * This plugin is forked from `rollup-plugin-svelte`
 * @see rollup-plugin-svelte
 */
 ...
 */
module.exports = function cachedSvelte(options = {}) {
  /** @type {Map<string, {mtime:number;size:number;result:any}>} */
  ...
  return {
    ...
    async transform(code, id) {
      ...
      if (c) {
        const st = stat(id);
        if (st && c.mtime === st.mtimeMs && c.size === st.size) {
          // return cache
          if (this.addWatchFile && Array.isArray(c.result.dependencies)) {
            c.result.dependencies.forEach(this.addWatchFile);
          }
          return c.result;
        }
      }
      ...
      if (elapsed > 10) {
        ...
        if (st) {
          ...
          // console.log(`cached ${id} (saving ${elapsed.toFixed(0)}ms)`)
        }
      }
      ...
      // console.log('svelte done +' + (performance.now() - t).toFixed(2) + 'ms');
      return compiled.js;
    },
