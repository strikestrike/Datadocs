...
class HistoryManager {
  ...
  undoToActionId(actionId: string, refresh: boolean = true) {
    if (this.isProcessing) {
      return;
    }
    this.isProcessing = true;
    ...
    while (action) {
      if (!action.hidden) {
        action.undo(action.id === actionId && refresh);
        redoActions.push(action);
  ...
  redoToActionId(actionId: string, refresh: boolean = true) {
    if (this.isProcessing) {
      return;
    }
    this.isProcessing = true;
    ...
    while (action) {
      if (!action.hidden) {
        action.redo(action.id === actionId && refresh);
        undoActions.push(action);
        if (action.id === actionId) {
          break;
        }
