<script lang="ts">
  ...
  import { blankCanvasConfig } from "../../../_dprctd-panels-sheet/BlankCanvas/BlankCanvas";
  import { graphConfig } from "../../../_dprctd-panels-sheet/Graph/Graph";
  ...
  const settings: PaneSettings = pane?.settings;

  const activeTab: View = settings?.tabs[settings.active || 0];
    ...
    if (activeTab) {
      ...
      if (target) {
        ...
        if (pane?.settings) {
          ...
          if (pane?.settings?.tabs) {
            if (pane?.settings?.tabs[pane?.settings?.active || 0]) {
              pane.settings.tabs[pane?.settings?.active || 0] = {
                ...activeTab,
              };
            }
          }
...
<div
  class={clsx(
    ...
    settings?.tabAsView ? "top-[45px] right-[15px]" : "top-[35px] right-[5px]"
  )}
