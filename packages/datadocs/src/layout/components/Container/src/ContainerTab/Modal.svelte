<script lang="ts">
  ...
  import FlexPane from "../../../../pages/workbook/components/Main/index.svelte";
  ...
  const closeModal: () => {} = getContext(CLOSE_MODAL_CONTEXT_NAME);
  const action: DND = DND.WORKBOOK_TAB;
  ...
  function onDragOver({ detail }: { detail: MouseEvent }) {
    isDragOverTop = true;
    if ($appDnd && elementIndicators && elementDrag) {
      const bounds = elementIndicators.getBoundingClientRect();
      const boundsDrag = elementDrag.getBoundingClientRect();
      let left = $appDnd.event.clientX - $appDnd.data.offsetLeft - bounds.left;
    ...
    Object.keys(childrenBounds).forEach((id) => {
      ...
      if (
        detail.clientX > bounds.x + 10 &&
        detail.clientX < bounds.x + bounds.width
      ) {
        const childrenCache = [...pane.children];
        let targetIndex = childrenCache.findIndex((child) => child.id === id);
        const dragIndex = childrenCache.findIndex(
          (child) => child.id === dragId,
        );
        targetIndex = targetIndex === -1 ? childrenCache.length : targetIndex;
        if (dragIndex > -1) {
          const temp = childrenCache.splice(dragIndex, 1);
          childrenCache.splice(targetIndex, 0, ...temp);
        } else {
          childrenCache.splice(targetIndex, 0, dragPane);
        }
...
<div class="modal" style="--modal-header-height: 46px;">
  <div class="modal-header">
    <div class="tabs relative">
      <div
        ...
        class="tab-indicators flex items-end pt-1 h-[var(--modal-header-height)] px-2.5 relative flex-shrink-0 z-1"
      >
        {#each children as child, i (child.id)}
          ...
          >
            <Tab
              ...
              {action}
              className={activeId === child.id
                ? "!h-9 mr-0.5"
                : "!h-8 !bg-[#a7b0b533] mr-0.5"}
              classNameCorner={activeId === child.id ? "" : "hidden"}
      ...
      >
        <div class="absolute w-full h-[80px] top-0 z-110" />
      </Drop>
    </div>
  ...
  <div class="modal-body">
    <FlexPane pane={children[activeIndex]} />
  </div>
</div>
