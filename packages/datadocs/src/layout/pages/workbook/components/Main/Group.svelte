<script lang="ts">
  ...
  import {
    ContainerTab,
    ContainerTabCollapse,
  } from "src/layout/components/Container";
  import { writable } from "svelte/store";
  import FlexPane from "./index.svelte";
  ...
  // This is for special collapse group, maybe move it to HGroup and VGroup
  let children = [];
  $: {
    if (!isCollapse(pane)) {
      ...
      children = pane.children.map((child) => {
        if (!isVGroup(child) && !isHGroup(child)) {
          return child;
        } else {
          const newChild = findFirstChildDeepById(
            child.id,
            (config) => !isVGroup(config) && !isHGroup(config),
          );
          return {
            ...cloneDeep(newChild),
            id: child.id,
  ...
  $: {
    // console.log(isVGroup(pane), pane);
  }
</script>
...
<div
  ...
  class={clsx(
    "pane pane-group box-border relative w-full h-full",
    {
      "h-group": isHGroup(pane),
    },
    {
      "v-group": isVGroup(pane),
    },
    {
      "vertical-tabs": isHGroup(parent) && isTabsGroup(pane),
    },
    {
      "fixed-group": isFixedGroup(pane),
    },
    {
      "last-pane": getNextById(pane.id) === null,
    },
...
>
  {#if isHGroup(pane) || isVGroup(pane)}
    {#each children as innerPane, i (innerPane.id)}
      <FlexPane pane={innerPane} />
      {#if i < children.length - 1}
        <PaneDivider
          pane={innerPane}
          data-dndtype="after-group"
          orientation={isHGroup(pane)
